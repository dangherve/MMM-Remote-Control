...
async function readModuleData (baseDirectory, modulesDirectory, onModuleLoaded) {
  ...
  try {
    await fs.access(path.resolve(`${baseDirectory}/../../defaultmodules`), fs.constants.F_OK);
  ...
  const files = await fs.readdir(path.resolve(`${baseDirectory}/..`));
...
async function installModule (options) {
  const {url, baseDirectory, onSuccess, onError} = options;

  try {
    // Clone repository using native promises
    await simpleGit(path.resolve(`${baseDirectory}/..`)).clone(url, path.basename(url));

    const workDirectory = path.resolve(`${baseDirectory}/../${path.basename(url)}`);
...
async function updateModule (options) {
  const {moduleName, baseDirectory, modulesAvailable, onSuccess, onError} = options;

  Log.log(`UPDATE ${moduleName || "MagicMirror"}`);

  let modulePath = `${baseDirectory}/../../`;
  let name = "MM";
  ...
  if (moduleName && moduleName !== "undefined") {
    ...
    modulePath = `${baseDirectory}/../${moduleData.name}`;
